...
namespace TaskPilot.Web.APIs
{
    ...
    public class ChartController : ControllerBase
    {
        ...
        public IEnumerable<ChartDTO> GetDueTasks()
        {
            ...
            var dueTasks = _unitOfWork.Tasks.GetAll().GroupBy(x => x.DueDate).Select(x => new { Date = x.Key, DueTasksCount = x.Count() }).ToList();
            var createdTask = _unitOfWork.Tasks.GetAll().GroupBy(x => x.Created.Date).Select(x => new { Date = x.Key, CreatedTasksCount = x.Count() }).ToList();
            var resolvedTask = _unitOfWork.Tasks.GetAllInclude(null, "Status").Where(x => x.Status!.Description == "Closed").GroupBy(x => x.Updated!.Value.Date).Select(x => new { Date = x.Key, ResolvedTasksCount = x.Count() }).ToList();
